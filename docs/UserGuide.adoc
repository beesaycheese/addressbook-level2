...
Sorts the list of persons in alphabetical order without actually sorting the actual list.
Format: `sort`
...
* `sort` +
`list` +
Returns a list of persons in alphabetical order. But still returns an unsorted list if the command `list` is used.
